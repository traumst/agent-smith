...
- [ ] Initialize Go module (`go mod init smithai`).
- [ ] Create initial folder structure (`cmd/smith`, `internal/agent/protocol`, `internal/persistence`).
- [ ] Define core protocol types in `internal/agent/protocol` (e.g. `Request`, `Response`, `SystemPrompt`, `FileDelta`, `ToolDef`, `ToolCall`).
- [ ] **Wiring:** Create a minimal `cmd/smith/main.go` that imports these types and prints a dummy request to verify the build.

### Step 1.2: Settings Management
- [ ] Implement `internal/persistence/settings` to manage agent configuration.
- [ ] Create functions to read/write settings (Competence, Mood, Instructions) as JSON from/to disk. Ensure settings are passed explicitly via function arguments (no globals).
- [ ] **Wiring:** Update `main.go` to load settings from disk on startup and print the configured mood.

### Step 1.3: Relational Storage (SQLite)
- [ ] Set up `mattn/go-sqlite3` DB connection logic (CGO enabled).
- [ ] Implement schema and basic CRUD for `chat_history` (`internal/persistence/history`).
- [ ] Implement schema and basic CRUD for `usage_logs` (`internal/persistence/logs`).
- [ ] Implement schema and basic CRUD for the `references` table (`internal/persistence/refs`).
- [ ] **Wiring:** Update `main.go` to initialize the DB, write a test chat message, and query it back.

### Step 1.4: Vector Storage & Long-Term Memory
- [ ] Integrate `sqlite-vec` extension loading via `go-sqlite3`.
- [ ] Implement `internal/persistence/vector` for vectorized keyword lookups.
- [ ] Implement logic to manage capped plaintext memory files in `data/memory/` and register them in the `references` table.
- [ ] **Wiring:** Update `main.go` to create a dummy memory file, vectorize its keywords, and perform a similarity search.
